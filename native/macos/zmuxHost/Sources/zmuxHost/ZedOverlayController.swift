...
final class ZedOverlayController: NSObject {
    ...
    func configure(_ command: ConfigureZedOverlay) {
        ...
        if enabled {
            ...
            handleActivatedApplication(NSWorkspace.shared.frontmostApplication, userInitiatedActivation: false)
        } else {
    ...
    private func handleActivatedApplication(_ application: NSRunningApplication?, userInitiatedActivation: Bool) {
        guard enabled else {
            return
        }
        /**
         ...
         */
        BrowserOverlayRestoreReproLog.append("zedOverlay.activation", [
    ...
    private func showWindow() {
        ...
        BrowserOverlayRestoreReproLog.append("zedOverlay.showWindow.request", [
            "isWindowVisibleInAttachment": isWindowVisibleInAttachment,
            "shouldRestoreWindowWhenZedReturns": shouldRestoreWindowWhenZedReturns,
            "visibleWindowFrame": visibleWindowFrame.map { "x=\($0.minX),y=\($0.minY),w=\($0.width),h=\($0.height)" } as Any,
            "windowFrame": "x=\(window.frame.minX),y=\(window.frame.minY),w=\(window.frame.width),h=\(window.frame.height)",
        ])
        ...
        isWindowVisibleInAttachment = true
        didShowAttachment()
        BrowserOverlayRestoreReproLog.append("zedOverlay.showWindow.applied", [
            "isWindowVisibleInAttachment": isWindowVisibleInAttachment,
            "windowFrame": "x=\(window.frame.minX),y=\(window.frame.minY),w=\(window.frame.width),h=\(window.frame.height)",
        ])
        updateButtonPosition()
    ...
    private func moveWindowOffscreen(openWorkspaceInZed shouldOpenWorkspaceInZed: Bool) {
        ...
        BrowserOverlayRestoreReproLog.append("zedOverlay.moveWindowOffscreen.request", [
            "isWindowVisibleInAttachment": isWindowVisibleInAttachment,
        ...
        didHideAttachment()
        BrowserOverlayRestoreReproLog.append("zedOverlay.moveWindowOffscreen.applied", [
            "isWindowVisibleInAttachment": isWindowVisibleInAttachment,
            "sourceFrame": "x=\(sourceFrame.minX),y=\(sourceFrame.minY),w=\(sourceFrame.width),h=\(sourceFrame.height)",
            "windowFrame": "x=\(window.frame.minX),y=\(window.frame.minY),w=\(window.frame.width),h=\(window.frame.height)",
        ])
    ...
    private func updateButtonTitle() {
         ...
         */
        toggleButton?.title = isWindowVisibleInAttachment ? "Show \(targetAppConfiguration().buttonName)" : "Show zmux"
    }
    ...
    private func readTargetWindowFrame() -> TargetWindowFrame? {
        ...
        guard isAccessibilityTrusted(promptIfNeeded: shouldPromptForAccessibility) else {
             ...
             */
            BrowserOverlayRestoreReproLog.append("zedOverlay.readTargetWindowFrame.accessibilityDenied", [
                "targetApp": targetApp.rawValue,
            ])
            Self.logger.info("Accessibility permission is required before the target editor frame can be read")
            return nil
        }
    ...
    private func pushHiddenWindowFurtherLeftAfterDelay() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) { [weak self] in
            ...
            BrowserOverlayRestoreReproLog.append("zedOverlay.moveWindowOffscreen.delayedLeftNudge", [
                "windowFrame": "x=\(window.frame.minX),y=\(window.frame.minY),w=\(window.frame.width),h=\(window.frame.height)",
            ])
        }
    }
    ...
    private func screen(containingAccessibilityTopLeft point: CGPoint) -> NSScreen? {
        ...
        return NSScreen.screens.first { screen in
            let xMatches = appKitPoint.x >= screen.frame.minX - 80 && appKitPoint.x <= screen.frame.maxX + 80
            let yMatches = appKitPoint.y >= screen.frame.minY - 80 && appKitPoint.y <= screen.frame.maxY + 80
    ...
    private func targetAppConfiguration() -> TargetApp {
        switch targetApp {
        case .zed:
            TargetApp(buttonName: "Zed", bundleName: "Zed.app", commandName: "zed", commandReuseWindowArgument: "--existing", processName: "Zed")
        case .zedPreview:
            TargetApp(buttonName: "Zed", bundleName: "Zed Preview.app", commandName: "zed", commandReuseWindowArgument: "--existing", processName: "Zed Preview")
        case .vscode:
            TargetApp(buttonName: "VS Code", bundleName: "Visual Studio Code.app", commandName: "code", commandReuseWindowArgument: "--reuse-window", processName: "Code")
        case .vscodeInsiders:
            TargetApp(buttonName: "VS Code", bundleName: "Visual Studio Code - Insiders.app", commandName: "code-insiders", commandReuseWindowArgument: "--reuse-window", processName: "Code - Insiders")
        }
    }
