...
final class BrowserOverlayController {
    ...
    func open(_ command: OpenBrowserWindow) {
        appendReproLog("open.request", [
            "url": command.url,
            "state": stateSnapshot(),
        ])
        guard let url = URL(string: command.url) else {
            appendReproLog("open.invalidUrl", [
                "url": command.url,
                "state": stateSnapshot(),
            ])
            return
        ...
        guard let applicationURL = chromeCanaryApplicationURL() else {
            ...
            appendReproLog("open.canaryAppMissing", [
                "state": stateSnapshot(),
            ])
            return
        }
        ...
        NSWorkspace.shared.open([url], withApplicationAt: applicationURL, configuration: configuration) { [weak self] app, error in
            DispatchQueue.main.async {
                ...
                if let error {
                    Self.logger.error("Failed to open Chrome Canary: \(error.localizedDescription, privacy: .public)")
                    self.appendReproLog("open.failed", [
                        "error": error.localizedDescription,
                        "state": self.stateSnapshot(),
                    ])
                    return
                }
                self.appendReproLog("open.workspaceOpened", [
    ...
    func moveBrowserOffscreen() {
        appendReproLog("moveOffscreen.request", [
            "state": stateSnapshot(),
        ])
        guard isBrowserVisibleInAttachment else {
            appendReproLog("moveOffscreen.skippedNotVisibleInAttachment", [
                "state": stateSnapshot(),
            ])
            return
        }
        guard let application = activeBrowserApplication ?? runningChromeCanaryApplication(),
              let chromeWindow = activeBrowserWindow ?? frontmostChromeCanaryWindow(for: application) else {
            appendReproLog("moveOffscreen.skippedNoWindow", [
                "state": stateSnapshot(),
            ])
            return
        }
        ...
        guard let frame else {
            appendReproLog("moveOffscreen.skippedNoFrame", [
        ...
        appendReproLog("moveOffscreen.applied", [
    ...
    func restoreBrowserIfNeeded() {
        appendReproLog("restoreIfNeeded.request", [
            "state": stateSnapshot(),
        ])
        guard isBrowserMovedOffscreen else {
            appendReproLog("restoreIfNeeded.skippedNotMovedOffscreen", [
                "state": stateSnapshot(),
            ])
            return
        }
    ...
    func showRunningChromeCanary() {
        appendReproLog("showRunning.request", [
            "state": stateSnapshot(),
        ])
        guard let application = runningChromeCanaryApplication() else {
            appendReproLog("showRunning.skippedNoRunningApp", [
                "state": stateSnapshot(),
            ])
            return
        }
    ...
    func markBrowserNoLongerShownInAttachment(reason: String) {
         ...
         */
        appendReproLog("markBrowserNoLongerShown.request", [
            "reason": reason,
            "state": stateSnapshot(),
        ])
        isBrowserVisibleInAttachment = false
        isBrowserMovedOffscreen = false
        appendReproLog("markBrowserNoLongerShown.applied", [
            "reason": reason,
            "state": stateSnapshot(),
        ])
    }
    ...
    private func chromeCanaryApplicationURL() -> URL? {
        if let url = NSWorkspace.shared.urlForApplication(withBundleIdentifier: Self.chromeCanaryBundleIdentifier) {
            return url
        }
        ...
        return NSWorkspace.shared.runningApplications
            .first { $0.bundleIdentifier == Self.chromeCanaryBundleIdentifier || $0.bundleURL?.lastPathComponent == Self.chromeCanaryAppName }?
            .bundleURL
    }

    private func positionChromeCanary(app: NSRunningApplication?, remainingAttempts: Int) {
        appendReproLog("position.request", [
        ...
        guard remainingAttempts > 0 else {
            ...
            appendReproLog("position.failedNoAttempts", [
                "state": stateSnapshot(),
            ])
            return
        }
        ...
        guard let application else {
            appendReproLog("position.retryNoApplication", [
                "remainingAttempts": remainingAttempts,
                "state": stateSnapshot(),
            ])
        ...
        guard let frame = workareaFrameProvider() ?? window?.frame,
              let chromeWindow = frontmostChromeCanaryWindow(for: application) else {
            appendReproLog("position.retryNoFrameOrWindow", [
                "remainingAttempts": remainingAttempts,
        ...
        appendReproLog("position.applied", [
    ...
    private func retryPositioning(app: NSRunningApplication?, remainingAttempts: Int) {
        DispatchQueue.main.asyncAfter(deadline: .now() + Self.windowPlacementRetryDelay) { [weak self] in
    ...
    private func openNewTabForRunningChromeCanary(_ application: NSRunningApplication) {
        appendReproLog("showRunning.openNewTab.request", [
        ...
        guard let applicationURL = chromeCanaryApplicationURL() ?? application.bundleURL else {
            appendReproLog("showRunning.openNewTab.missingAppUrl", [
                "appPid": application.processIdentifier,
                "state": stateSnapshot(application: application),
            ])
            return
        ...
        let configuration = NSWorkspace.OpenConfiguration()
        configuration.activates = true
        NSWorkspace.shared.open([Self.chromeCanaryNewTabURL], withApplicationAt: applicationURL, configuration: configuration) { [weak self] app, error in
            DispatchQueue.main.async {
                guard let self else {
                    return
                }
                if let error {
                    Self.logger.error("Failed to create Chrome Canary tab: \(error.localizedDescription, privacy: .public)")
                    self.appendReproLog("showRunning.openNewTab.failed", [
                        "error": error.localizedDescription,
                        "state": self.stateSnapshot(application: application),
                    ])
                    return
                }
                self.appendReproLog("showRunning.openNewTab.opened", [
                    "appPid": app?.processIdentifier as Any,
    ...
    private func frontmostChromeCanaryWindow(for application: NSRunningApplication) -> AXUIElement? {
        ...
        if AXUIElementCopyAttributeValue(axApplication, kAXFocusedWindowAttribute as CFString, &focusedWindowValue) == .success,
    ...
    private func stateSnapshot(
        ...
        let application = suppliedApplication ?? activeBrowserApplication ?? runningChromeCanaryApplication()
    ...
    private func describeChromeWindowFrame(_ chromeWindow: AXUIElement) -> String? {
        ...
        guard AXUIElementCopyAttributeValue(chromeWindow, kAXPositionAttribute as CFString, &positionValue) == .success,
              AXUIElementCopyAttributeValue(chromeWindow, kAXSizeAttribute as CFString, &sizeValue) == .success,
              let positionValue,
              let sizeValue else {
            return nil
        }
...
enum BrowserOverlayRestoreReproLog {
    ...
    static func append(_ event: String, _ details: [String: Any] = [:]) {
        ...
        do {
            ...
            logger.warning("failed to write browser overlay restore repro log: \(error.localizedDescription)")
        }
    }
