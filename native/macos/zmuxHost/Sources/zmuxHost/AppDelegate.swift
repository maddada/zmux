...
final class AppDelegate: NSObject, NSApplicationDelegate, GhosttyAppDelegate {
    static let logger = Logger(subsystem: "com.zmux.host", category: "app")
    ...
    private static func mergeGhosttyTerminalSettings(
        ...
        let lines = retainedLines + [
            "",
            "# Config generated by zmux terminal settings",
...
private final class NativeSettingsStore {
    private static let logger = Logger(subsystem: "com.zmux.host", category: "settings")
...
final class zmuxRootView: NSView {
    private static let logger = Logger(subsystem: "com.zmux.host", category: "webview")
    ...
    private let workspaceBarSeparator = NSBox()
    ...
    /**
     ...
     Native zmux keeps the project/workspace rail in its own webview to the left
     of the sidebar, and uses an AppKit drag handle to resize only the sidebar
    ...
    init(
        ...
        configuration.userContentController.add(scriptBridge, name: "zmuxWorkspaceBar")
        ...
        let workspaceBarConfiguration = WKWebViewConfiguration()
        workspaceBarConfiguration.userContentController.add(scriptBridge, name: "zmuxWorkspaceBar")
        ...
        self.workspaceBarView = WKWebView(frame: .zero, configuration: workspaceBarConfiguration)
        ...
        sidebarCommandRouter.onWorkspaceBarMessage = { [weak self] body in
            self?.handleWorkspaceBarMessage(body)
        }
        ...
        workspaceBarView.setValue(false, forKey: "drawsBackground")
        ...
        workspaceBarSeparator.boxType = .custom
        workspaceBarSeparator.fillColor = NSColor.separatorColor
        workspaceBarSeparator.borderColor = .clear
         ...
         */
        addSubview(workspaceBarView)
        addSubview(workspaceBarSeparator)
    ...
    override func layout() {
        ...
        let chromeWidth = Self.workspaceBarWidth + separatorWidth + sidebarWidth + Self.dividerWidth
        ...
        workspaceBarView.frame = CGRect(
            x: chromeX,
            y: 0,
            width: Self.workspaceBarWidth,
            height: bounds.height
        )
        workspaceBarSeparator.frame = CGRect(
            x: chromeX + Self.workspaceBarWidth,
            y: 0,
            width: separatorWidth,
            height: bounds.height
        )
        sidebarView.frame = CGRect(
            x: chromeX + Self.workspaceBarWidth + separatorWidth,
            y: 0,
            width: sidebarWidth,
            height: bounds.height
        )
        divider.frame = CGRect(
            x: chromeX + Self.workspaceBarWidth + separatorWidth + sidebarWidth,
            y: 0,
    ...
    private func handleWorkspaceBarMessage(_ body: Any) {
        ...
        switch type {
        case "workspaceBarState":
            postWorkspaceBarState(message)
        case "workspaceBarReady":
            requestWorkspaceBarState()
        case "focusProject":
            guard let projectId = message["projectId"] as? String,
                  let data = try? JSONEncoder().encode(projectId),
                  let json = String(data: data, encoding: .utf8) else {
                return
            }
    ...
    private func requestWorkspaceBarState() {
        sidebarView.evaluateJavaScript("window.__zmux_NATIVE_WORKSPACE_BAR__?.getState();") { [weak self] result, _ in
            guard let self,
                  let state = result as? [String: Any] else {
                return
            }
            self.postWorkspaceBarState(state)
        }
    }

    private func postWorkspaceBarState(_ state: [String: Any]) {
        guard JSONSerialization.isValidJSONObject(state),
              let data = try? JSONSerialization.data(withJSONObject: state),
              let json = String(data: data, encoding: .utf8) else {
            return
        }
        workspaceBarView.evaluateJavaScript("""
        window.dispatchEvent(new CustomEvent('zmux-workspace-bar-state', { detail: \(json) }));
        """)
    }

    private func loadWorkspaceBar() {
        workspaceBarView.loadHTMLString(Self.workspaceBarHTML, baseURL: nil)
    }
...
final class SidebarScriptBridge: NSObject, WKScriptMessageHandler {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if message.name == "zmuxWorkspaceBar" {
            router.onWorkspaceBarMessage?(message.body)
            return
        }
...
final class SidebarCommandRouter {
    ...
    var onWorkspaceBarMessage: ((Any) -> Void)?
}
