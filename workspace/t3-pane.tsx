...
const T3_CHAT_MARKDOWN_STYLE_ELEMENT_ID = "zmux-t3-chat-markdown-zoom";
...
export const T3Pane: React.FC<T3PaneProps> = ({
  ...
  useEffect(() => {
    ...
    const resolveClipboardImagePath = async (
      ...
      const vscodeApi = window.__zmux_WORKSPACE_VSCODE__;
      if (!vscodeApi) {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.type === "zmuxT3ThreadChanged") {
        const sessionId =
      ...
      if (event.data.type === "zmuxT3WorkingStartedAtChanged") {
        const sessionId =
          typeof event.data.sessionId === "string" ? event.data.sessionId.trim() : "";
      ...
      if (event.data.type === "readNativeClipboardPayloadResult") {
        ...
        const payloadFiles = Array.isArray(event.data.files)
          ? event.data.files
              .map((file: unknown): T3ClipboardFilePayload | null => {
                const candidate =
                  file && typeof file === "object"
                    ? (file as { buffer?: unknown; name?: unknown; type?: unknown })
                    : undefined;
                const resultBuffer: ArrayBuffer | undefined =
                  candidate?.buffer instanceof ArrayBuffer
                    ? candidate.buffer
                    : ArrayBuffer.isView(candidate?.buffer)
                      ? new Uint8Array(
                          candidate.buffer.buffer,
                          candidate.buffer.byteOffset,
      ...
      if (event.data.type === "zmuxT3DebugLog") {
        const sessionId =
          typeof event.data.sessionId === "string" ? event.data.sessionId.trim() : "";
        const logEvent =
      ...
      if (event.data.type === "zmuxT3ClipboardWrite") {
        const text = typeof event.data.text === "string" ? event.data.text : "";
        void navigator.clipboard.writeText(text).catch(() => {});
        return;
      }

      if (event.data.type === "zmuxT3ClipboardWriteRequest") {
        const requestId =
          typeof event.data.requestId === "number" ? event.data.requestId : undefined;
        const text = typeof event.data.text === "string" ? event.data.text : "";
            ...
            iframeRef.current?.contentWindow?.postMessage(
              {
                ok: true,
                requestId,
                type: "zmuxT3ClipboardWriteResult",
              },
              "*",
            );
          },
      ...
      if (event.data.type === "zmuxT3ClipboardReadRequest") {
        const requestId =
          typeof event.data.requestId === "number" ? event.data.requestId : undefined;
        if (requestId === undefined) {
          return;
          ...
          iframeRef.current?.contentWindow?.postMessage(
            {
              ...
              requestId,
              text,
              type: "zmuxT3ClipboardReadResult",
            },
            "*",
            transferables,
          );
...
function readFrameWindowState(frameWindow: Window | null | undefined): Record<string, unknown> {
  ...
  try {
    return {
      activeThreadId:
        (frameWindow as Window & { __zmux_T3_ACTIVE_THREAD_ID__?: string })
          .__zmux_T3_ACTIVE_THREAD_ID__ ?? undefined,
