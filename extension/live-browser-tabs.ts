...
function inspectLiveBrowserTab(tab: vscode.Tab): LiveBrowserTabInspection {
  ...
  if (tab.label.trim().startsWith(zmux_SEARCH_LABEL_PREFIX)) {
    return { accepted: false, inputKind, reason: "ignored:zmux-search-label" };
  }
  ...
  if (iszmuxOwnedTab(tab, viewType, url)) {
    return {
      accepted: false,
      inputKind,
      labelUrl,
      rawUrl,
      reason: "ignored:zmux-owned",
      url,
      viewType,
    };
  }
...
function iszmuxOwnedTab(
  ...
  if (normalizedViewType?.startsWith(zmux_VIEW_TYPE_PREFIX)) {
    return true;
  }

  if (tab.label.trim().toLowerCase() !== zmux_LABEL) {
    return false;
  }

  return url === undefined || iszmuxLocalAssetUrl(url);
}
...
function iszmuxLocalAssetUrl(url: string): boolean {
