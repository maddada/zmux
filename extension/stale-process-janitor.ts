...
export async function runStaleVsmuxProcessJanitor(
  ...
  logzmuxDebug("janitor.staleProcesses.completed", {
    killedProcessCount: staleProcessIds.length,
    killedProcessIds: staleProcessIds,
    storagePath: context.globalStorageUri.fsPath,
  });
...
export async function listStaleVsmuxProcessIds(
  ...
  try {
    ...
    logzmuxDebug("janitor.staleProcesses.psFailed", {
  ...
  return processes
    .filter((processInfo) => {
      ...
      const hasVsmuxEnvironmentMarkers =
        normalizedCommand.includes("zmux_agent=") ||
        normalizedCommand.includes("zmux_session_state_file=") ||
        normalizedCommand.includes("zmux_wrapper_pid=") ||
        normalizedCommand.includes("zmux_vscode_app_root=") ||
      ...
      const isOldExtensionHelper =
        isHelperProcess &&
        normalizedCommand.includes("/maddada.zmux-") &&
