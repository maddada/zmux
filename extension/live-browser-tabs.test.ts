...
describe("getLiveBrowserTabs", () => {
  ...
  test("should detect embedded Storybook urls on unknown-input tabs with noisy query separators", () => {
    // CDXC:PublicRelease 2026-04-27-05:50 Keep public test fixtures on generic private LAN examples instead of captured local host addresses.
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
          {
            ...
            isActive: true,
            label:
              "Sidebar / Interactions - Toolbar Actions - Storybook (http://192.168.1.50:6007|? path%3D%2Fstory%2Fsidebar-interactions--toolbar-actions)",
          },
        ],
        viewColumn: 1,
      } as never,
    ...
    expect(browserTabs[0]).toEqual(
      expect.objectContaining({
        detail: "http://192.168.1.50:6007?path%3D%2Fstory%2Fsidebar-interactions--toolbar-actions",
        inputKind: "undefined",
        url: "http://192.168.1.50:6007?path%3D%2Fstory%2Fsidebar-interactions--toolbar-actions",
      }),
    );
  });
  ...
  test("should ignore zmux T3 webviews", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
          {
            input: new vscode.TabInputWebview("zmux.t3Session"),
            isActive: true,
            label: "T3",
          },
        ],
  ...
  test("should ignore tabs whose title starts with zmux Search", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
          {
            input: new vscode.TabInputWebview("workbench.zmuxsearch.panel"),
            isActive: true,
            label: "zmux Search - Network",
          },
        ],
        viewColumn: 1,
      } as never,
  ...
  test("should ignore the zmux workspace webview", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
  ...
  test("should ignore restored zmux custom tabs without an http url", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
  ...
  test("should ignore zmux localhost asset tabs", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
  ...
  test("should still allow real browser tabs titled zmux", () => {
    const browserTabs = getLiveBrowserTabs([
      {
        isActive: true,
        tabs: [
          {
            input: new vscode.TabInputCustom(
              vscode.Uri.parse("https://example.com/zmux"),
              "simpleBrowser.view",
            ),
            isActive: true,
            label: "zmux",
