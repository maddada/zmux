...
describe("parseAgentControlChunk", () => {
  test("should strip agent lifecycle markers from visible terminal output", () => {
    const parsed = parseAgentControlChunk(`hello\u001b]9001;zmux;start;codex\u0007world`);
  ...
  test("should buffer incomplete lifecycle markers until the terminator arrives", () => {
    const firstChunk = parseAgentControlChunk(`busy\u001b]9001;zmux;stop;codex`);

    expect(firstChunk.output).toBe("busy");
    expect(firstChunk.pending).toBe(`\u001b]9001;zmux;stop;codex`);
...
describe("getClaudeHookSettingsContent", () => {
  test("should wire Claude lifecycle hooks into the shared notifier", () => {
    const settings = JSON.parse(getClaudeHookSettingsContent("/tmp/zmux-notify.sh", "linux")) as {
      hooks: Record<
        string,
  ...
  test("should emit a Windows-safe command when requested", () => {
    const settings = JSON.parse(
      getClaudeHookSettingsContent("C:\\temp\\zmux-notify.cmd", "win32"),
    ) as {
      hooks: Record<
        string,
        Array<{ hooks: Array<{ command: string; type: string }>; matcher?: string }>
...
describe("getZshRcShimContent", () => {
  test("should make CLAUDE_BIN point at the zmux Claude wrapper for user profile aliases", () => {
    const content = getZshRcShimContent("/tmp/zmux/bin");
    const expectedExport = "export CLAUDE_BIN='/tmp/zmux/bin/claude'";
    ...
    expect(content.lastIndexOf(expectedExport)).toBeGreaterThan(
      content.indexOf("export PATH='/tmp/zmux/bin':$PATH"),
    );
  });
});
...
describe("getOpenCodePluginContent", () => {
  test("should schedule a non-blocking sync of the current session status", () => {
    const plugin = getOpenCodePluginContent("/tmp/zmux-notify.js", "/usr/local/bin/node");

    expect(plugin).toContain("const currentSessionId = process?.env?.zmux_SESSION_ID;");
...
describe("getPowerShellBootstrapContent", () => {
  test("should persist and re-emit the current PowerShell title", () => {
    ...
    expect(content).toContain("function global:__zmux_sync_current_title()");
    expect(content).toContain("function global:zmux_set_title");
    expect(content).toContain('[System.Console]::Out.Write("$([char]27)]0;');
    expect(content).toContain("__zmux_write_session_title $title");
