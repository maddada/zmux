...
describe("first prompt auto rename repro log", () => {
  ...
  afterEach(async () => {
    process.env.zmux_DEBUGGING_MODE = "true";
  ...
  test("writes events into a dedicated workspace .zmux file", async () => {
    ...
    expect(logPath).toBe(path.join(workspaceRoot, ".zmux", "first-prompt-auto-rename-repro.log"));
  ...
  test("does not create the log file when debugging mode is disabled", async () => {
    ...
    process.env.zmux_DEBUGGING_MODE = "false";
